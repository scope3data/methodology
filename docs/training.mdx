...
[Carbon Emissions and Large Neural Network Training](https://arxiv.org/pdf/2104.10350) includes a table that shows the impact of changing the model, the datacenter, the gpu, and the energy grid in training a model:
...
Infrastructure data:
- [Training cluster](/cluster) details
...
For 1,082,990 GPU hours and 2832 hours (118 days) of training time, training emissions were:

```
EmbEm(training) = (7.27 kgCO2e/h) x (2832 h)
                = 20,589 kgCO2e

OpEm(training)  = (.471 kW) x (1082990 h) x (57 kgCOw2/kWh)
                = 29,075 kgCO2e

Em(training)    = 49.7 mtCO2e
```
...
| Manufacturing WUE | 147 L/GPU |

Modeling the cluster produces:
```
EmbH2O(h) = ((384 GPUs) x (147 L/GPU))
            / (4 years)
            / (8760 hours/year)
            / (95% utilization)
          = 1.7 L/h
...
Manufacturing H20 = (2832 h) x (1.7 L/h) = 4.8 kL
```
