...
| ----------------------- | ------------------------------------------------------------------------------------------------ |
| `default`               | Uses data not specific to this entity                                                            |
| `spend-corporate`       | Uses spend-based data at the corporate level                                                     |
| `spend-biz-unit`        | Uses spend-based data at the business unit level                                                 |
| `activity-3p-observed`  | Activity data observed from a third-party source (for instance, data.ai)                         |
| `activity-1p-estimated` | Activity data provided by the entity based on estimates (for instance, average screen power use) |
| `activity-1p-measured`  | Activity data provided by the entity based on measured data (for instance, power meter or GA)    |
...
| `model_quality`                                 | Yes      | Source of the property model                                                                   |
...
| `model_quality`                                 | Source of the ad platform model                                                             |
...
| `model_quality` | Source of the ad stack model              |
...
### Media Delivery - Model Quality

```
media_delivery_model_quality = switch property.model_quality:
    /activity-1p/:           0.5
    /activity-3p-estimated/: 0.75
    /activity-3p-measured/:  1
    default:                 0
```
...
```
unique_ad_platform_set = {}

get_platform_emissions(ad_platform):
  unique_ad_platform_set.add(ad_platform)
...
### Ad Selection - Model Quality

```
modeled_platforms = 0
for platform in unique_ad_platform_set:
    modeled_platforms += '/activity/ ' in platform.model_quality ? 1 : 0

ad_selection_model_quality =
    '/activity/ ' in platform.model_quality ?
    modeled_platforms / unique_ad_platform_set.length :
    0
```
