...
For each audio broadcast method, we assume a fixed energy consumption required to transmit the signal from the [BBC](https://downloads.bbc.co.uk/rd/pubs/whp/whp-pdf-files/WHP393.pdf) study, regardless of the number of radios receiving it.
...
For example (BBC), for FM the total reported annual energy consumption is `26 GWh`, covering eight national stations and 41 local stations. We assume that the coverage of ten local stations is equivalent to that of one national station.
