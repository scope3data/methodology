...
Note that we do not include racks, networking gear, or other datacenter infrastructure in the definition of the cluster; this should probably be improved in a future version. The PUE/WUE for the datacenter do account for usage energy from the cooling infrastructure.
...
- The idle power draw of the server (see [Cloud Carbon Footprint](https://www.cloudcarbonfootprint.org) for common cloud instances)
...
- The projected use life of the server (up to 6 years for cloud platforms, but suggest using 3-4 years for AI instances given pace of change)
